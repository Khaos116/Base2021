...
class StickyAnyDecoration extends RecyclerView.ItemDecoration {
  ...
  private void cacaheHeaderView(RecyclerView parent, int position, ViewParams view) {
    if (!mHeaderCache.containsKey(position)) {
      final RecyclerView.ViewHolder holder = mAdapter.createViewHolder(parent, position);
    ...
    if (adapterPosList.size() < 1) {
      if (childCount >= 1) {
        ...
        ViewParams viewParams =
            new ViewParams((int) child.getY(), child.getLeft(), child.getMeasuredWidth(), child.getMeasuredHeight());
