...
public class SimpleViewpagerIndicator extends HorizontalScrollView {
  ...
  //不允许选中的位置
  private int mNoClickPosition = -1;
  //点击不允许的时候吐司
  private String mNoClickMsg = "";
  ...
  public SimpleViewpagerIndicator setNoClickPosition(int position, String msg) {
    this.mNoClickPosition = position;
    this.mNoClickMsg = msg;
    return this;
  }
    ...
    public void onPageSelected(int position) {
      if (position == mNoClickPosition) {
        if (position > selectedPosition) { //往左滑,向右选中
          ...
          int count = 0;
          if (mAdapter != null) count = mAdapter.getCount();
          if (position + 1 < count) viewPager.setCurrentItem(position + 1);
