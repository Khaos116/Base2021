...
class VideoControllerView @JvmOverloads constructor(
  ...
  defStyleRes: Int = 0
    ...
    private fun showView() {
        if (isShowInoperableView) return
    ...
    private fun hiddenView() {
        controller_bottom_progressbar?.animate()?.alpha(if (isShowInoperableView) 0f else 1f)
            ?.start()
    ...
    //是否显示了不可操作的View
    private var isShowInoperableView = false
    override fun callShowInoperableView(show: Boolean) {
        isShowInoperableView = show
        if (show) {
            job?.cancel()
            jobLock?.cancel()
            hiddenView()
    ...
    override fun callOverDoubleClick() {
        if (isLocked || isShowInoperableView) return
        controllerListener?.onPlayOrPause()
    }
